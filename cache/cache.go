...
import (
	...
	"github.com/opensourceways/app-community-metadata/helper"
	"go.uber.org/zap"
)
...
func InitCache() {
	conf := app.Config.StringMap("cache")
	prefix := conf["prefix"]
	server := conf["server"]
	password := conf["auth"]
	redisDB, _ := strconv.Atoi(conf["db"])
	color.Info.Printf("============ Cache server=%s, db=%d ============\n", server, redisDB)
	Init(NewRedisPool(server, password, redisDB), prefix, app.Logger, app.Debug)
}
